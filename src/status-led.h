...
const struct RGB locked_rgb {196, 62, 29};

// Function defitions

void init_status_led_timer(struct RGB init_status_led)
{
    ...
    TB3CCR1 = 4*init_status_led.red;    // set CCR1 to red duty cycle
    TB3CCTL1 |= CCIE;                   // enable compare interrupt
    TB3CCTL1 &= ~CCIFG;                 // clear interrupt flag

    TB3CCR2 = 4*init_status_led.green;  // set CCR2 to green duty cycle
    TB3CCTL2 |= CCIE;                   // enable compare interrupt
    TB3CCTL2 &= ~CCIFG;                 // clear interrupt flag

    TB3CCR3 = 4*init_status_led.blue;   // set CCR3 to blue duty cycle
...
void status_led_timer_ccr1(void)
{
...
void status_led_timer_ccr2(void)
{
    // set red color to low
    P2OUT &= ~BIT1;
    TB3CCTL2 &= ~CCIFG;                 // clear interrupt flag
...
void status_led_timer_ccr3(void)
{
    // set red color to low
    P2OUT &= ~BIT2;
    TB3CCTL3 &= ~CCIFG;                 // clear interrupt flag
